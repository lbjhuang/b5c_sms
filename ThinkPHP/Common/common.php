...
function A($name, $layer = '', $common = false)
{
    ...
    } elseif (C('APP_GROUP_LIST') && count($path) < 4) {
        $name = '@/' . $layer . '/' . GROUP_NAME . '/' . $path[2];
        import($name . $layer);
    } else {
        import($name . $layer);
    }
 ...
 * @return mixed
 */
function L($name = null, $value = null,$clear = false)
{
    static $_lang = array();
    if (true == $clear) {
        $_lang = [];
        ...
        if (is_null($value))
            if (cookie('think_language') == 'zh-cn') {
                ...
                if (isset($_lang[$name])) {
                    return $_lang[$name];
                    #兼容英文=>英文
                } elseif (isset($_lang[strtoupper($name)])) {
                    return $_lang[strtoupper($name)];
                } else {
                    return $name . '[*待译]';
                }
                // return isset($_lang[$name]) ? $_lang[$name] : $name . '[*待译]';
                //return !empty($_lang[$name]) ? $_lang[$name] : $name;
            }
 ...
 * @return string
 */
function guid($no_symbol = false) {
    $charid = md5(uniqid(mt_rand(), true));
    $hyphen = chr(45);// "-"
    if($no_symbol) return $uuid = substr($charid, 0, 8).$hyphen
