...
class Model {
    ...
    public function parseFieldsMap($data,$type=1) {
        ...
        if(!empty($this->_map)) {
            foreach ($this->_map as $key=>$val){
                if($type==1) { // 读取
                    if(isset($data[$val]) || array_key_exists($val,$data)) {
                        $newData[$key] =   $data[$val];
                        unset($data[$val]);
                    }
                }else{
                    if(isset($data[$key])|| array_key_exists($val,$data)) {
     ...
     * @return Model
     */
    public function cache($key=true,$expire=3,$type=''){
        if(false !== $key)
     ...
     * @return Model
     */
    public function where($where,$parse=null,$join = false){
        ...
        if (is_string($where) && ('' === $where || null === $where)) {
            return $this;
        }elseif(is_string($where) && '' != $where){
            $map    =   array();
            if($join && is_string($this->options['where']['_string']) && '' != $this->options['where']['_string']){
                $map['_string']   = $this->options['where']['_string'].' AND '. $where;
            }else{
                $map['_string']   =   $where;
            }
            $where  =   $map;
    ...
    {
        $map = array();
        if (is_string($this->options['where']['_string']) && '' != $this->options['where']['_string']) {
            $map['_string'] = $this->options['where']['_string'] . ' AND ' . $where;
        }else{
            $map['_string']   =   $where;
        }
    ...
    public function setEnforcement($state)
    {
        $this->db->enforcement = (boolean)$state;
    }
