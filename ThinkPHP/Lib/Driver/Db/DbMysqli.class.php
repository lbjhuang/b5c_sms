...
class DbMysqli extends Db{
    ...
    public function rollback() {
        --$this->transTimes;
        if ($this->transTimes == 0 || true === $this->enforcement) {
    ...
    {
        //Fixed 修复批量插入验证错误的bug, Afanti @2017-12-14；当数据是关联索引数组时，验证不通过，框架设计比较坑。
        //原验证方法：if(!is_array($datas[0])) return false;
        //原字段方法：$fields = array_keys($datas[0]);
        if (empty($datas) || !is_array(reset($datas))) return false;
        ...
        $sql .= $this->parseTable($options['table']) . ' (' . implode(',', $fields) . ') VALUES ';
        $sql .= implode(',', $values);
        return $this->execute($sql);
    }
