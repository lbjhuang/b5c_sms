const warehouseRecommendedNew = Vue.component("warehouseRecommendedNew", {
    template: `
            <div class="warehouse-recommended-new">
                 <div class="edit">
                                   ...
                                   <div class="conditions__being-selected">
                                       ...
                                       <div class="conditions__being-selected-content">
                                         <template v-for="(item,index) in selectedConditionals">
                                           <div :key="index" class="selected__conditional" @click="onSelect(item)">
...
<!--   需求不明确，暂时取消                                            <el-checkbox  v-if="storePlatform.is_gp===1" v-model="deliveryWarehouse" :label="$lang('用户指定的发货仓库：指定的发货仓库')"></el-checkbox>-->
    ...
    created() {
        // 商品SKU
        this.$bus.$on("add-all-selected-sku", (value) => {
        ...
        // 商品包含的SKU前缀
        this.$bus.$on("add-all-selected-sku-prefix", (value) => {
            console.log("监听到了", value);
        ...
        // 商品包含的SKU后缀
        this.$bus.$on("add-all-selected-sku-suffix", (value) => {
            console.log("监听到了", value);
            if (!value) {
        ...
        // 选择国家
        this.$bus.$on("add-all-order-destination", (value) => {
            console.log("监听到了", value);
            if (value.length === 0) {
                this.allOrderDestination = [];
                this.selectedConditionals.forEach((obj) => {
    ...
    methods: {
        ...
        getWarehousesByStoreId() {
            axios.post("/index.php?m=store&a=logisticsCompanyByStore", {store_id: this.storeId}).then((res) => {
                ...
                console.log("仓库数据", this.allWarehouses);
                console.log("物流公司", this.allLogisticsCompanyByWarehouseCode);
            })
        },
        onSave() {
            this.loading = true;
            ...
            this.$refs["form"].validate((valid) => {
                if (valid) {
                    axios.post("/index.php?m=store&a=customWarehouseCreate", {
                        store_id: this.storeId,
                        ...
                        console.log("规则新增成功", res.data);
                        this.$message.success(this.$lang('新增成功'));
                        this.$emit("new-saved", false);
                        this.loading = false;
        ...
        onSelect(item) {
            if (item.identification === this.initConditionals[0].identification) {
                this.$emit('open-goods-sku-dialog-box-by-add', true);
                this.$nextTick(() => {
                    this.$bus.$emit('get-goods-sku-by-add', this.allSku);
                    console.log("触发了事件");
                })
            } else if (item.identification === this.initConditionals[1].identification) {
                this.$emit('open-goods-sku-prefix-dialog-box-by-add', true);
                this.$nextTick(() => {
                    this.$bus.$emit('get-goods-sku-prefix-by-add', this.allSkuPrefix);
                    console.log("触发了事件");
                })
            } else if (item.identification === this.initConditionals[2].identification) {
                this.$emit('open-goods-sku-suffix-dialog-box-by-add', true);
                this.$nextTick(() => {
                    this.$bus.$emit('get-goods-sku-suffix-by-add', this.allSkuSuffix);
                    console.log("触发了事件");
                })

            } else if (item.identification === this.initConditionals[3].identification) {
                this.$emit('open-order-destination-dialog-box-by-add', true);
                this.$nextTick(() => {
                    this.$bus.$emit('get-order-destination-by-add', this.allOrderDestination);
                    console.log("触发了事件");
                })
            } else if (item.identification === this.initConditionals[4].identification) {
                this.$emit('open-delivery-warehouse-dialog-box-by-add', true);
                this.$nextTick(() => {
                    this.$bus.$emit('get-delivery-warehouse-by-add', this.allDeliveryWarehouse);
                    console.log("触发了事件");
                })
            } else if (item.identification === this.initConditionals[5].identification) {
                this.$emit('open-mode-distribution-dialog-box-by-add', true);
                this.$nextTick(() => {
                    this.$bus.$emit('get-mode-distribution-by-add', this.allModeDistribution);
                    console.log("触发了事件");
                })
            } else if (item.identification === this.initConditionals[6].identification) {
                this.$emit('open-delivery-country-dialog-box-by-add', true);
                this.$nextTick(() => {
                    this.$bus.$emit('get-delivery-country-by-add', this.allDeliveryCountry);
                    console.log("触发了事件");
                })
            }
        },
        onClose() {
            this.$emit("new-closed");
        }
    }
});