const warehouseRecommendedGoodsSku = Vue.component("warehouseRecommendedGoodsSku", {


    template: `
<div class="goodsSkuNew">
  <div class="list">
    <div class="goods__sku">
     ...
     <div class="list__filters">
      ...
      <div class="goods__sku-selected-content">
        ...
        >
          <template slot-scope="scope">
            <span>{{ $lang(scope.row.sku_states)}}</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    created() {
        ...
        this.$bus.$on("get-goods-sku-by-add", (values) => {
    ...
    beforeDestroy() {
        this.$bus.$off("get-goods-sku-by-add")
    },
    methods: {//这里定义的组件的方法，利用$emit()进行父子组件通信，子组件通过点击事件告诉父组件触发一个自定义事件，$emit()方法第二个参数也可以用来传递数据
        ...
        onSave() {
            if (this.toBeAllSelectedSkuStatus.length === 0) {
                this.$emit('closed-goods-sku', false)
                this.$bus.$emit("add-all-selected-sku", this.toBeAllSelectedSkuStatus)
        ...
        search() {
            ...
            axios.get(`${this.pmsHost}/product/index`, {
                ...
                console.log("PMS列表数据", this.pagedGoodsSku);
            });
        }
    }
});