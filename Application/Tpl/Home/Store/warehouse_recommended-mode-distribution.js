const warehouseRecommendedModeDistribution = Vue.component("warehouseRecommendedModeDistribution", {
    template: `
<div class="mode-distribution">
  <div class="edit">
    ...
    created() {
        this.$bus.$on("get-mode-distribution-by-add", (values) => {
    ...
    methods: {//这里定义的组件的方法，利用$emit()进行父子组件通信，子组件通过点击事件告诉父组件触发一个自定义事件，$emit()方法第二个参数也可以用来传递数据
        ...
        onSave() {
            let allSelectedModeDistribution = _.cloneDeep(this.allSelectedModeDistribution);
            this.$bus.$emit("add-all-mode-distribution", allSelectedModeDistribution);
            this.$emit('closed-mode-distribution', false)
        ...
        onSearch() {
            //正则表达式
            console.log(this.filter.userDeliveryMode);
            console.log(this.filter.logisticsMode);
