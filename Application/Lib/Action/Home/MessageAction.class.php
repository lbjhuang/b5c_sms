...
class MessageAction extends BaseAction {
    ...
    public function choosenewuserall() {
        ...
        //default where
        $whereby = array_merge($whereby,$where);
        $datas = D('ZZmsthrdcust')->find_msg_user(1,null,null,$where,null);
        $count = $datas['count'];
        $ajaxoutputs= array('info'=>'','status'=>'n');
        if($this->isAjax()){
            $ajaxoutputs['status'] = 'y';
    ...
    public function choosenewuserlist() {
        ...
        $filterArr = array();
        $filterArr['msg_id']                = I('msg_id');
        $filterArr['by_parent_plat_cd']     = I('by_parent_plat_cd');
        $filterArr['keyword_type']          = I('keyword_type');
        $filterArr['keywords']              = I('keywords');
        $filterArr['is_select']             = I('is_select');// whether selected or not
        ...
        if($filterArr['is_select']){
            $where['_string'] = " CUST_ID IN (
                SELECT CUST_ID COLLATE utf8_unicode_ci FROM ".D('ZZmsmessagesend')->getTableName()." WHERE ".D('ZZmsmessagesend')->getTableName().".CUST_ID=".D('ZZmsthrdcust')->getTableName().".CUST_ID COLLATE utf8_unicode_ci AND msg_id='".$msg_id."'
            ) ";
        }
        ...
        $datas = D('ZZmsthrdcust')->find_msg_user(0,$firstRow,$listRows,$where,null);
        $count = $datas['count'];
        $list = $datas['list'];
    ...
    public function submitadd() {
        ...
        if($issubmit==1){
            ...
            //send
            $tmp = D('ZZmsmessages')->do_one_send($msg_id);
            if($this->isAjax()){
                $ajaxoutputs['status'] = 'y';
                echo json_encode($ajaxoutputs);
                die();
    ...
    public function submitedit() {
        ...
        if($issubmit==1){
            ...
            //send
            $tmp = D('ZZmsmessages')->do_one_send($edit_id);
