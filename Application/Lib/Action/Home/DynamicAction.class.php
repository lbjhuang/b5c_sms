...
//        $this->upd_safes();
        ...
        $where_stand['tb_wms_center_stock.total_inventory'] = array('neq', 0);
        $where_stand['tb_wms_center_stock.CHANNEL_SKU_ID'] = array('eq', 0);
        $show = null;
        if ('down' != I('post.down')) {
            if (!empty(I("SKU"))) {
            ...
            /*   if (I("channel") == 'false') {
               ...
               if (I("channel") == 'true') {
                   if (I('all_channel')) {
                       $where_stand['tb_wms_center_stock.channel'] = array('eq', I('all_channel'));
                   }
            ...
            if ($this->action['ks_ico'] == 'arrow-down-b') {
                $order = 'tb_wms_center_stock.sale desc';
            } else {
                $order = 'tb_wms_center_stock.sale asc';
            }
        }
            ...
            if (I("inventory_status") == 5) {
                if (!empty(I("GUDS_CNS_NM")) || !empty(I("DELIVERY_WAREHOUSE"))) {
                    $count_all = $Stand->cache(30)->where($where_stand)
                        ->join('left join tb_wms_early on tb_wms_early.SKU = tb_wms_center_stock.SKU_ID')
                        ...
                        ->field('tb_wms_center_stock.SKU_ID')
                        ->count();
                } else {
                    $count_all = $Stand->cache(30)->where($where_stand)
                        ->join('left join tb_wms_early on tb_wms_early.SKU = tb_wms_center_stock.SKU_ID')
                ...
                $sel_h = $Stand->where($where_stand)
                    ->join('left join tb_wms_early on tb_wms_early.SKU = tb_wms_center_stock.SKU_ID AND tb_wms_early.channel = tb_wms_center_stock.channel')
                    ->field('tb_wms_center_stock.SKU_ID,tb_wms_center_stock.channel')
                    ->select();
                ...
                if (!empty(I("GUDS_CNS_NM")) || !empty(I("DELIVERY_WAREHOUSE"))) {
                    $count = $Model->table($early_sql . ' es')->where($where_stand)
                        ->join('left join tb_wms_center_stock on es.SKU = tb_wms_center_stock.SKU_ID AND es.channel = tb_wms_center_stock.channel')
                        ->join('left join (select * from tb_ms_guds group by tb_ms_guds.DELIVERY_WAREHOUSE,tb_ms_guds.MAIN_GUDS_ID) tmg  on tmg.MAIN_GUDS_ID = tb_wms_center_stock.GUDS_ID')
                        ->field('tb_wms_center_stock.total_inventory')
                        ->count();
                } else {
                    $count = $Model->table($early_sql . ' es')->where($where_stand)
                        ->join('left join tb_wms_center_stock on es.SKU = tb_wms_center_stock.SKU_ID AND es.channel = tb_wms_center_stock.channel')
        ...
        if ('down' != I('post.down')) {
            ...
            if (!empty(I("inventory_status"))) {
                $sql_s = $model_s->table('tb_wms_center_stock')->where($where_stand)
                    ...
                    ->join('left join tb_wms_early on tb_wms_early.SKU = tb_wms_center_stock.SKU_ID AND tb_wms_early.channel = tb_wms_center_stock.channel')
                    ->join('left join  (select tb_ms_guds.DELIVERY_WAREHOUSE,tb_ms_guds.MAIN_GUDS_ID from tb_ms_guds group by tb_ms_guds.DELIVERY_WAREHOUSE,tb_ms_guds.MAIN_GUDS_ID) tmg  on tmg.MAIN_GUDS_ID = tb_wms_center_stock.GUDS_ID')
                    ->order($order)
                    ->select(false);
...
//        $res = $this->getpin($start_date, $end_date);
        ...
        $where['tb_wms_center_stock.total_inventory'] = array('neq', 0);
        //            is no channel
        $where['tb_wms_center_stock.channel'] = array('EQ', 'N000830100');
        if ($sale_state == 1) {
            $where['tb_wms_center_stock.SKU_ID'] = array('IN', array_column($res, 'sku_id'));
            $data_arr = $standing = $Standing->where($where)
                ->join('left join  (select tb_ms_guds.DELIVERY_WAREHOUSE,tb_ms_guds.MAIN_GUDS_ID from tb_ms_guds group by tb_ms_guds.DELIVERY_WAREHOUSE,tb_ms_guds.MAIN_GUDS_ID) tmg  on tmg.MAIN_GUDS_ID = tb_wms_center_stock.GUDS_ID')
                ->join('left join tb_wms_power on tb_wms_power.SKU_ID = tb_wms_center_stock.SKU_ID')
                ->field('tb_wms_center_stock.*,tmg.DELIVERY_WAREHOUSE as warehouse_id,tb_wms_power.weight')
                ->select();
        } else {
            if (count($res)) {
                $where['tb_wms_center_stock.SKU_ID'] = array('NOT IN', array_column($res, 'sku_id'));
            }
            $data_arr = $standing = $Standing->where($where)
                ->join('left join tb_wms_power on tb_wms_power.SKU_ID = tb_wms_center_stock.SKU_ID')
                ->join('left join  (select tb_ms_guds.DELIVERY_WAREHOUSE,tb_ms_guds.MAIN_GUDS_ID from tb_ms_guds group by tb_ms_guds.DELIVERY_WAREHOUSE,tb_ms_guds.MAIN_GUDS_ID) tmg  on tmg.MAIN_GUDS_ID = tb_wms_center_stock.GUDS_ID')
                ->field('tb_wms_center_stock.*,tb_wms_power.weight,tmg.DELIVERY_WAREHOUSE as warehouse_id')->select();
