...
class CustomerAction extends BaseAction {
    ...
    public function kehulist(){
        ...
        $whereby['parent_plat_cd'] = array('IN', ZFun::arrToIdsArr(array(PLAT_B5C_CUSTOMER)));
        ...
        if($filterArr['regstarttime'] or $filterArr['regendtime']) {
            if($filterArr['regstarttime'] && $filterArr['regendtime']) {
                $where['JOIN_REQ_DT'] = array(array('gt',($filterArr['regstarttime'] . ' 00:00:00')),array('lt',($filterArr['regendtime'] . ' 23:59:59')));
            } elseif($filterArr['regstarttime']) {
                $where['JOIN_REQ_DT'] = array('gt', ($filterArr['regstarttime'] . ' 00:00:00'));
            } elseif($filterArr['regendtime']) {
                $where['JOIN_REQ_DT'] = array('lt', ($filterArr['regendtime'] . ' 23:59:59'));
            }
        }

        if($filterArr['loginstarttime'] or $filterArr['loginendtime']) {
            if($filterArr['loginstarttime'] && $filterArr['loginendtime']) {
                $where['SYS_REG_DTTM'] = array(array('gt',($filterArr['loginstarttime'] . ' 00:00:00')),array('lt',($filterArr['loginendtime'] . ' 23:59:59')));
            } elseif($filterArr['loginstarttime']) {
                $where['SYS_REG_DTTM'] = array('gt', ($filterArr['loginstarttime'] . ' 00:00:00'));
            } elseif($filterArr['loginendtime']) {
                $where['SYS_REG_DTTM'] = array('lt', ($filterArr['loginendtime'] . ' 23:59:59'));
            }
        }
        ...
        //所属平台
        $plat_list = D("ZZmscmncd")->gainCustomerSites();
     ...
     *
     */
    public function show_customer_extend()
    {
        $params = ZUtils::filterBlank($this->getParams());
        // 用户基本信息
        $customerInfo   = $this->getCustomerInfo($params ['ID']);
        // 用户订单信息
        $customerOrders = $this->getCustomerOrder($params);
        // 用户交易金额信息
        $this->getCustomerPayInfo($params);

        $this->baseData();
        $this->assign('result', $customerInfo);
        $this->assign('orders', $customerOrders);
        $this->assign('count', count($customerOrders));
     ...
     * @return array|false $ret        用户信息
     */
    public function getCustomerInfo($customerId)
    {
        $model = new BTBTCModel();
        $ret = $model->where('id = %d', [$customerId])->find();

        return $ret;
    }

    const CUSTOMER_STATE_1 = 'N000550100'; //待确认
    const CUSTOMER_STATE_2 = 'N000550200'; //确认中
    const CUSTOMER_STATE_3 = 'N000550300'; //待付款
    const CUSTOMER_STATE_4 = 'N000550400'; //待发货
    const CUSTOMER_STATE_5 = 'N000550500'; //待收货
    const CUSTOMER_STATE_6 = 'N000550600'; //已收货
    const CUSTOMER_STATE_7 = 'N000550700'; //已付尾款
    const CUSTOMER_STATE_8 = 'N000550800'; //交易成功
    const CUSTOMER_STATE_9 = 'N000550900'; //交易关闭
    const CUSTOMER_STATE_10 = 'N000551000'; //交易取消

    /**
     * 需要获取所有的订单，累加总金额用于展示，并且不受筛选条件限制
     * @param array $params 筛选参数
     */
    public function getCustomerPayInfo($params)
    {
        $orderModel = M('_op_order', 'tb_');
        $conditions ['receiver_cust_id'] = ['eq', $params ['ID']];

        $fields = [
            'PAY_CURRENCY',//币种
            'BWC_ORDER_STATUS',//订单状态
            'ORDER_PAY_TIME',//支付时间
            'PAY_TOTAL_PRICE',//订单金额
        ];
        $orders = $orderModel->field($fields)
            ->where($conditions)
            ->select();

        $waitForPayMoney = $refundMoney = $payMoney = 0;

        $waitForPayState = [
            self::CUSTOMER_STATE_1,
            self::CUSTOMER_STATE_2,
            self::CUSTOMER_STATE_3
        ];

        $refundMoneyState = [
            self::CUSTOMER_STATE_4,
            self::CUSTOMER_STATE_5,
            self::CUSTOMER_STATE_6,
            self::CUSTOMER_STATE_7,
            self::CUSTOMER_STATE_8
        ];

        $payMoneyState = [
            self::CUSTOMER_STATE_9,
            self::CUSTOMER_STATE_10
        ];

        foreach ($orders as $key => $value) {
            $money = 0;
            $money = ((floatval($this->getCurrencyRate($value ['PAY_CURRENCY'], date('Ymd', strtotime($value ['ORDER_PAY_TIME']))))) * floatval($value ['PAY_TOTAL_PRICE']));
            if (in_array($value ['BWC_ORDER_STATUS'], $waitForPayState)) // 代付款金额
                $waitForPayMoney += $money;
            elseif (in_array($value ['BWC_ORDER_STATUS'], $refundMoneyState)) // 已支付金额
                $payMoney += $money;
            elseif (in_array($value ['BWC_ORDER_STATUS'], $payMoneyState)) // 已取消金额
                $refundMoney += $money;
        }

        $this->assign('waitForPayMoney', $waitForPayMoney);
     ...
     * 客户订单获取
     * @param  array      $params  筛选参数
     * @return array|false $orders 用户订单
     */
    public function getCustomerOrder($params)
    {
        $orderModel = M('_op_order_guds', 'tb_');
        $conditions ['t1.receiver_cust_id'] = ['eq', $params ['ID']];
        empty($params ['ORD_STAT_CD']) or $conditions ['t1.BWC_ORDER_STATUS'] = ['eq', $params ['ORD_STAT_CD']];
        //最大金额 sku
        $t3 = $orderModel->field('max(t2.ITEM_COUNT*t2.ITEM_PRICE)')
                        ->table('tb_op_order_guds t3')
                        ->where('t3.PLAT_CD = t1.PLAT_CD and t3.ORDER_ID = t1.ORDER_ID')
                        ->buildSql();
        $t2 = $orderModel->field('t2.SKU_ID')
                        ->table('tb_op_order_guds t2')
                        ->where('t2.PLAT_CD = t1.PLAT_CD AND t2.ORDER_ID = t1.ORDER_ID AND (t2.ITEM_COUNT * t2.ITEM_PRICE) = ' . $t3)
                        ->limit(0, 1)
                        ->buildSql();
        $fields = [
            't1.B5C_ORDER_NO',//B5C订单号
            't1.ORDER_ID',//第三方订单号
            't1.PLAT_CD',//消费平台
            't1.PAY_CURRENCY',//币种
            't1.PAY_TOTAL_PRICE',//订单金额(需要货币转换，并且减去优惠券金额)
            't1.BWC_ORDER_STATUS',//订单状态
            't1.ORDER_TIME',//下单时间
            't1.ORDER_PAY_TIME',//支付时间
            $t2 . 'as SKU_ID', // SKU
        ];
        $t1 = $orderModel->field($fields)
                        ->table('tb_op_order t1')
                        ->where($conditions)
                        ->buildSql();

        $count = $orderModel->field('t4.*, t5.GUDS_NM')
                            ->table($t1 . ' t4')
                            ->join('LEFT JOIN tb_ms_guds t5 ON SUBSTR(t4.SKU_ID, 1, 8) = t5.GUDS_ID')
                            ->count();

        $page = new Page($count, 20);
        $pages = $page->show();

        $orders = $orderModel->field('t4.*, t5.GUDS_NM')
            ->table($t1 . ' t4')
            ->join('LEFT JOIN tb_ms_guds t5 ON SUBSTR(t4.SKU_ID, 1, 8) = t5.GUDS_ID')
            ->limit($page->firstRow, $page->listRows)
            ->select();
    ...
    public function baseData()
    {
        ...
        $store = BaseModel::getBtbtcCustomerStore();
        $curre = BaseModel::getCurrency();
        $state = BTBTCModel::state();
        $plats = BTBTCModel::plat();
        $customerStore = BaseModel::getBtbtcCustomerStore();
        foreach ($state as $key => $value) {
            if (in_array($key, $needDelState))
                unset($state [$key]);
        }
        ...
        $this->assign('store', $store);
        $this->assign('curre', $curre);
        $this->assign('state', $state);
        $this->assign('plats', $plats);
        $this->assign('customerStore', $customerStore);
    }
