...
//            return false;
    ...
    public function overdue_mail_send()
    {
        ...
        $where['tb_b2b_receipt.transaction_type'] = array('EXP', 'IS NULL');
        $where['tb_b2b_receipt.actual_receipt_date'] = array('EXP', 'IS NULL');
        $where['tb_b2b_receipt.overdue_date'] = array('EXP', 'IS NOT NULL');
...
//        聚合
     ...
     * @return int
     */
    public static function actual_overdue_upd($id, $date, $Receipt, $todo = false)
    {

        $receipt_s = $Receipt->field('overdue_date,actual_receipt_date,ORDER_ID')->where('ID = ' . $id)->find();
        if ($todo && !$receipt_s['actual_receipt_date']) {
            if ($date < date('Y-m-d')) {
                $save['overdue_statue'] = $status = 1;
                $Receipt->where('ID = ' . $id)->save($save);
            }
        } else {
            if (empty($receipt_s['overdue_date']) || date('Y-m-d', strtotime($receipt_s['overdue_date'])) >= date('Y-m-d', strtotime($date))) {
     ...
     * @return int
     */
    public static function actual_overdue_order_upd($order_id, $Receipt = null)
    {
        if (empty($Receipt)) {
            $Receipt = M('receipt', 'tb_b2b_');
        }
...
//        if(count($mail_all['join']) > 0)static::mail_send(static::$mail['title30'], static::check_mail_message($mail_all['join']), NULL,'yangsu@gshopper.com');
     ...
     * @return mixed
     */
    public static function order_mail_send($order_id, $thr_po_id, $message, $user, $cc = null)
    {
        $thr_po_id = $thr_po_id ? $thr_po_id : '无';
        $title = 'ID：' . $thr_po_id . ' B2B Sales Order Details';
        if (APP_STATUS == 'stage') {
            $pre_msg = '[' . APP_STATUS . ']';
            $title = $pre_msg . $title;
            $message = $message . "\r\n<br>" . $pre_msg;
        }
        $mail_res = self::mail_send($title, $message, $cc, $user);
        return $mail_res;
    }
    ...
    {
        var_dump(B2bModel::toNode('{"nodei":0,"nodeType":"N001390400","nodeDate":"N001420800","nodeWorkday":"0","nodeProp":"100"}'));
    }
