...
class TrademarkService extends Service
{
    ...
    public function getTrademarkList($data, $is_excel = null)
    {
        $search_map = [
            ...
            'trademark_no' => 'tb_trademark_base.trademark_no',
            'created_by' => 'tb_trademark_base.created_by',
            'international_type' => 'tb_trademark_detail.international_type'
        ];
        list($wheres, $limit) = WhereModel::joinSearchTemp($data, $search_map, [], ['international_type', 'register_code', 'trademark_id', 'country_code', 'company_code', 'current_state']);
    ...
    public function getTrademarkListNew($data, $is_excel = null)
    {
        ...
        list($wheres, $limit) = WhereModel::joinSearchTemp($data, $search_map, [], ['international_type', 'register_code', 'trademark_id', 'country_code', 'company_code', 'current_state', 'current_step']);
        //截取满足条件的商标ID，这种方式怪怪的
        $trademark_ids = array_slice($all_trademark_ids, $limit[0], $limit[1]);
        $wheres['tb_trademark_base.id'] = ['IN', $trademark_ids];
        $wheres['tb_trademark_base.is_delete_state'] = '0';  //删除的不展示
        $res_return['data'] = $this->repository->getTrademarkListNew($wheres);
        if ($is_excel) {
            return $res_return['data'];
        }
...
//            $res_return['data'][$key]['goods'] = (array)$this->formatGoodsData($res_return['data'][$key]['goods']);
     ...
     * @throws Exception
     */
    public function getTrademarkDetail($trademark_id, $trademark_no)
    {
        return $this->repository->getTrademarkBaseAndDetail($trademark_id, $trademark_no);
    }
