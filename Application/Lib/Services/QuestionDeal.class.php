<?php
class QuestionDeal implements QuestionStrategy
{
	function dealWithButton($data = [])
	{
		...
		$data['status'] = $questionService::$STATUS_DONE;
		$data['opt_user_id'] = '';
		$data['opt_user_name'] = ''; 
		...
		$data['deal_user_id'] = ''; // 操作完后应处理人id(bbm_admin.M_ID)，多个时以英文逗号隔开 
		...
		$wx_user = $question_log_model->getAllHistUser($data, true);
