...
{   
     ...
     * ExcelOperationModel constructor.
     * @param string $excel_path Excel文件路径
     * @throws Exception
     */
    public function __construct($excel_path)
    {
        if (in_array($this->detectUploadFileMIME($excel_path), $this->extension) and $this->detectUploadFileMIME($excel_path) !== 0) {
     ...
     * 
     */
    private function detectUploadFileMIME($excel_path) {
        $file = $_FILES ['file'];
        $flag = 0;
