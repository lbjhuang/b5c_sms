...
class BrandCateModel extends RelationModel
{
    ...
    */
     ...
     * @param array $condition ['selected','createSub']
     * @return mixed|string
     */
    private function getMaxCatCode($sellerId, $condition)
    {
        ...
        //拆解类目层级
        $first = $second = $third = "";
        if (!empty($condition['selected'])){
            $first = substr($selected, 0, 3);
            $second = substr($selected, 3, 3);
            $third = substr($selected, 6, 4);
        }
        
        if (empty($selected) || ($second === '000' && $third === '0000')) //第一级的最大Code
        {
            if ( $isSub === 'Y') { //找二级类最大的CODE
                $last = $this->getMaxCodeByPatten($sellerId, "{$first}%0000");
                $newSecond = substr($last,3,3) + 1;
                $last = $first . sprintf('%03d', $newSecond) . $third;
            } else { //找一级最大CODE
                $last = $this->getMaxCodeByPatten($sellerId, "%0000000");
                $newFirst = substr($last, 1, 2) + 1;
                $last = 'C' . sprintf('%02d', $newFirst) . '0000000';
            }
        } elseif ($second !== '000' && $third === '0000') {
            if ( $isSub == 'Y') { //找三级类最大的CODE
                $last = $this->getMaxCodeByPatten($sellerId, "{$first}{$second}%");
     ...
     * @return mixed
     */
    private function getMaxCodeByPatten($sellerId, $patten)
    {
        $where = "SLLR_ID = '{$sellerId}' ";
        !empty($patten) && $where .= "AND CAT_CD LIKE '{$patten}'";
        $last = $this->where($where)
            ->order('CAT_CD DESC')
            ->limit(1)
            ->getField('CAT_CD');
    ...
    public function deleteCate($condition)
    {
        ...
        if (!empty($condition['subCatPrefix'])){
            $where .= "AND `CAT_CD` LIKE '%{$condition['subCatPrefix']}%'";
        } else { //只删除三级类目，直接 等于就可以了，不要like.
            $where = "SLLR_ID='{$condition['sellerId']}' AND `CAT_CD` = '{$condition['selected']}'";
        }
