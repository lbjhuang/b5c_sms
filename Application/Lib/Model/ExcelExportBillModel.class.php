...
class ExcelExportBillModel extends BaseImportExcelModel
{
    private $reqData;
    ...
    public function fieldMapping($mode)
    {
        $rules = [
            'default' => [
                ...
                'remark' => ['field_name' => '备注', 'required' => true]//24 10766 调拨单待提交页面更改调拨商品 需求补充备注必填 20201126
    ...
    public function baseCode()
    {
        ...
        static::$baseCode = $model->table('tb_ms_cmn_cd')
            ->where('CD LIKE "N00124%" OR CD LIKE "N00323%" OR CD LIKE "N00068%" OR CD LIKE "N002460%" OR CD LIKE "N00129%" OR CD LIKE "N00128%" OR CD LIKE "N000950%"')
            ->getField('CD, CD_VAL');
    ...
    public function import()
    {
        ...
        if ($this->errorinfo) {
            ...
            if ($this->errorinfo) {
                ...
                $res = (new WmsModel())->xlsOutStorage($this->reqData);
                if (!$res || $res['code'] != 2000) {
                    ...
                    $response ['code'] = 2000;
                    $response ['info'] = L('成功出库：') .($this->excel->max_row - 1). L('条');
                    $response ['data'] = null;
                }
            }
        }
...
class ImportRequestModel extends AbstractRequestModel
{
     ...
     */
    public function outStorage($requestData)
    {
        ...
        if (!empty($response)) {
            ...
            foreach ($response as $key => $value) {
                ...
                foreach ($value->exportDetail as $batchKey => $batch) {
                    ...
                    $r ['add_time']              = date('Y-m-d H:i:s', substr($tmp->addTime, 0, 10));
                    $r ['digit']                 = $tmp->digit;
                    $r ['currency_time']         = date('Y-m-d H:i;s', substr($tmp->currencyTime, 0, 10));
                    ...
                    $r ['log_currency']          = $tmp->logCurrency;
