...
class TbOpOrdModel extends Model
{
    ...
    //发货
    public function sendOut($order_id,$plat_id=null,$ord_type=null) {
        ...
        if($set_stock_res['status'] == 'y' || $set_stock_res['code']=='x01') {
            ...
            trace($result['ResultCode'],'$result[\'ResultCode\']');
            if ($result['ResultCode'] == 0) { 
                ...
                if($order['B5C_ORDER_NO'] || 'new_self' == $ord_type){
                    if('new_self' == $ord_type && !$order['B5C_ORDER_NO']){
                        $order['B5C_ORDER_NO'] = $order_id;
                    }
    ...
    public function setStock($order_id) {
        $sms2_url = SMS2_URL;
        if ($_SERVER['HTTP_HOST'] == 'sms2.b5c.com') $sms2_url = 'http://sms2.b5c.com/';
