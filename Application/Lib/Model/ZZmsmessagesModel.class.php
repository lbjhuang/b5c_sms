...
class ZZmsmessagesModel extends CommonModel{
    ...
    public static function getStatusForMsg($key = null)
    {
        $items = [
            self::Msg_del => L('已删除'), 
            self::Msg_draft => L('草稿'),
            self::Msg_ongoing => L('进行中'),
            self::Msg_sent => L('已发送'),
            self::Msg_fail => L('发送失败'),
        ];
    ...
    {
        $items = [
            self::Send_part => L('部分客户'), 
            self::Send_all => L('所有用户'),
        ];
        return DataMain::getItems($items, $key);
    }
     ...
     *
     */
    public function do_one_send($msg_id){
        $ret = null;
        $one = $this->field ( '*' )
    ...
    public function one_msg_to_send($val){
        ...
        if(!isset($chk_data)){
            ...
            if($chk_data['send_status']!=3){
                if($timenow-$chk_data['create_time']>60){
                    $can_do = true;
                }
            }
        }
        ...
        if($can_do){
            ...
            if($msg_info['is_all_send']==1){
                $tmp = $this->do_send_push($msg_info,array());
                $ret_check = @json_decode($tmp,true);
                $ret['results'][] = $ret_check;
                $is_error = false;
                if($ret_check['code']!=2000){
                    $is_error = true;
                }
                if(!$is_error)
                ...
                if($arr){
                    ...
                    do{
                        ...
                        $tmp = $this->do_send_push($msg_info,$user_ids);
                        $ret_check = @json_decode($tmp,true);
                        $ret['results'][] = $ret_check;
                        $is_error = false;
                        if($ret_check['code']!=2000){
        ...
        // todo - need check results ok or not
        $check_send_status = $this->check_send_status($ret['results']);
        if($can_do){
            if($check_send_status=='error'){
                $status = $this->where(array('id'=>$msg_id))->data(array('send_status'=>self::Msg_fail))->save();
            }else{
                $status = $this->where(array('id'=>$msg_id))->data(array('send_status'=>self::Msg_sent))->save();
            }
        }
    ...
    public function do_send_push($msg_info,$user_ids){
        ...
        // trace('[create api - start]', str_replace("\\/", "/", json_encode(array('url'=>$url,'postArr'=>$postArr))));
        $res = ZWebHttp::getCurlHtml($url,$postArr);
        trace('[create api - end]', str_replace("\\/", "/", json_encode($res)));
        Log::write(
            '[debug api - push]'.str_replace("\\/", "/", json_encode(array($url,$arr,$res['html'])))
        );
        $htm = $res['html'];
        return $htm;
    }

    public function check_send_status($results){
        $ret = '';
