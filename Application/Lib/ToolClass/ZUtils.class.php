...
class ZUtils {
    ...
    */
    ...
    /**
     * 过滤空白
     * @param $param 被过滤参数可为字符串、数组
     * @return $param 过滤后数组或字符串
     */
    public static function filterBlank(&$param)
    {
        if (is_array($param)) {
     ...
     * @param int $delay  延迟多久
     * @return $script
     */
    public static function redirect_relay($url='', $delay = 2){
        echo '<script type="text/javascript">setTimeout(function() {location.href="' .$url. '"' . ';}, '. ($delay * 1000) .');</script>';
    }

    public static function mbtrim($val)
    {
        return preg_replace('/(^[\s\t\r\n　]+)|([\s\t\r\n　]+$)/u', '', $val);
    }

    public static function encodeDecodeId($act, $card)
    {
        $url = APP_STATUS == 'online' ? 'http://api.izene.org/card/' : 'http://172.16.111.93/card/';
        return HttpTool::curlReq($url, ['act' => $act, 'num' => $card]);
    }
     ...
     */
    public static function saveLog(array $array, $type, $file_pre = 'erp_due')
    {
        ...
        chmod($filePath, 0777);
        $fileName = $file_pre . '_' . date('Ymd') . '.log';
